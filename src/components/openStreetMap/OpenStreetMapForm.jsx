...
/**
 * Bản đồ OpenStreetMap nhúng trong form.
 * - `mode`: `picker` (create/edit — chọn vị trí) | `view` (chỉ xem).
 * - `picker`: controlled qua `lat`, `lng`, `onChange(lat, lng)`; search, GPS, click/kéo marker.
 * - `view`: chỉ hiển thị marker tại toạ độ; không search, không chỉnh vị trí.
 */
function OpenStreetMapForm({ mode = 'picker', lat, lng, onChange, className = '' }) {
  ...
  useEffect(() => {
    ...
    if (!markerRef.current) {
      loadLeaflet().then((L) => {
        ...
        markerRef.current = L.marker([propLat, propLng], {
          draggable: !isView,
  ...
  const showMissingCoords = isView && (propLat == null || propLng == null)
  ...
  return (
    <div className={className}>
      ...
      <div className="relative" style={{ zIndex: 0 }}>
        ...
        {(loading || error || showMissingCoords) && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/30 rounded-xl pointer-events-none">
            <div className="rounded-xl bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-700 dark:text-slate-200 shadow">
              {error ? (
                ...
                'Chưa có toạ độ để hiển thị trên bản đồ'
              ) : (
                'Đang tải bản đồ...'
              )}
            </div>
